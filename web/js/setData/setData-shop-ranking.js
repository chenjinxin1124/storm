...
function loadWebSocket_shop_ranking() {
    ...
    var web_socket = new WebSocket('ws://localhost:8888/web_socket/shop_ranking')
        ...
        for (var i = 0; i < sd.length; i++) {
           ...
           // alert(">>>"+shop_name);
            str += "<div class='row' style='margin-bottom:5px;margin-top:10px;line-height:30px;'>" +
                ...
                "</div>" +
                "<div class='col-md-4' style='width: 50%;background-color: rgba(8,158,195,0.2);text-align:left;color: #3dd3f8'>" +shop_name.get(sd[i].shop_id) + "</div>" +
                "<div class='col-md-4' style='width: 30%;text-align:center;line-height:30px;color: #3dd3f8'>" + sd[i].shop_amtSum + "</div>" +
                "</div>";
