...
public class KfkHBaseState implements State {

    private static final Logger LOG = LoggerFactory.getLogger(KfkHBaseState.class);

    private Options options;
    private KfkHBaseClient hBaseClient;
    private Map map;
    private int numPartitions;
    private int partitionIndex;

    protected KfkHBaseState(Map map, int partitionIndex, int numPartitions, Options options) {
    ...
    protected void prepare() {
        ...
        this.hBaseClient = new KfkHBaseClient(hbaseConfMap, hbConfig, options.tableName);
    ...
    public List<List<Values>> batchRetrieve(List<TridentTuple> tridentTuples) {
        ...
        for (TridentTuple tuple : tridentTuples) {
            byte[] rowKey = toBytes(tuple.get(0));
            Get get = new Get(rowKey);
             gets.add(get);
        }

        try {
            List<Values> list = Lists.newArrayList();
            Result[] results = hBaseClient.batchGet(gets);
            for(int i = 0; i < results.length; i++) {
                Map value = new HashMap ();
                for(Cell cell : results[i].rawCells()){
                    value.put(Bytes.toString(CellUtil.cloneQualifier(cell)),Bytes.toString(CellUtil.cloneValue(cell)));
                }

                list.add(new Values(value));

            }
            batchRetrieveResult.add(list);
        } catch (Exception e) {
